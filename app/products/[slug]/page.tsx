...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  ...
  // Resolve slug from async params (Next.js 15) and check context cache
  useEffect(() => {
    ...
    params.then(({ slug }) => {
      ...
      // 1. Try to find the product in the global context first (Cache hit)
      const cachedProduct = products.find(p => p.slug === slug);
      if (cachedProduct) {
        setProduct(cachedProduct);
        setSelectedSize(cachedProduct.sizes?.[0] ?? 'Free Size');
        setLoading(false);
        setNotFound(false); // Ensure notFound is false if product is found
        return; // FOUND IT — no need to call the server
      }

      // 2. If not found in cache and global loading is done, we must fetch it directly
      // This condition ensures we only fetch if the global product list has finished loading
      // and the product wasn't found there.
      if (!isLoading && !cachedProduct) {
        setIsLocalLoading(true);
        setLoading(true); // Set global loading to true while fetching individual product
  ...
  if (notFoundFlag || !product) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        <p className="text-emerald-900/60 font-serif italic text-xl">Product not found.</p>
        <button
          onClick={() => router.push('/shop')}
          className="bg-emerald-900 text-white px-8 py-3 rounded-full font-bold uppercase tracking-widest hover:bg-emerald-800 transition-all"
        >
          Back to Shop
        </button>
      </div>
  ...
  return (
    <div className="pb-32 bg-white min-h-screen">
      ...
      <div className="sticky top-0 z-40 lg:hidden glass flex items-center justify-between px-4 h-16">
        {/* BUG FIX: back button now uses router.back() preserving navigation history */}
